...
def is_enough_data_collected(result_file_path: Path, rounds: int):
    try:
        with open(result_file_path, "r", encoding="utf-8") as file:
            ...
            if len(reader) - 1 >= rounds or any("OOT" in row or "OOM" in row or "-" in row for row in reader):
                return True
...
def run_experiment(algo_settings: str, algo_name: str, graph_path: Path, grammar_path: Path, rounds: int, timeout_sec: Optional[int], result_file_path: Path):
    ...
    for _ in range(rounds):
        ...
        try:
            if os.path.isfile(running_file_path) and not user_confirms_rerun():
                ...
                result = run_appropriate_all_pairs_cflr_tool(
                    algo_settings=algo_settings, graph_path=graph_path, grammar_path=grammar_path, timeout_sec=timeout_sec
                )
            ...
            if e.returncode == TIMEOUT_EXIT_CODE:
                ...
                print(f"   Runner process terminated with return code {e.returncode}\n" f"   (interpreting as out of memory error)")
        ...
        with open(result_file_path, "a", newline="", encoding="utf-8") as csvfile:
            ...
            writer = csv.writer(csvfile)
            writer.writerow([algo_name, os.path.basename(graph_base_name), os.path.basename(grammar_base_name), s_edges, ram_kb, time_sec])
...
def reduce_result_file_to_one_row(result_file_path: Path) -> pd.DataFrame:
    ...
    df["ram_gb"] = df["ram_kb"].apply(lambda x: x / 10**6 if isinstance(x, (int, float)) else x)
    ...
    if df["s_edges"].isin(["OOM", "OOT", "-"]).any():
        ...
        if len(unique_s_edges) > 1:
            warnings.warn(f"Inconsistent 's_edges' values {unique_s_edges} " f"found in {result_file_path}. Using first 's_edges' value.")
        ...
        df = pd.DataFrame(
            {
                ...
                "ram_gb": [
                    (
                        ...
                        else f"{round_to_significant_digits(ram_gb_mean)}" f" ± {int(ram_gb_std / ram_gb_mean * 100)}%"
                    )
                ],
                "time_sec": [
                    ...
                    (
                        ...
                        else f"{round_to_significant_digits(time_sec_mean)}" f" ± {int(time_sec_std / time_sec_mean * 100)}%"
                    )
                ],
            }
        )
...
def display_results_for_grammar(df: pd.DataFrame, grammar: str):
    ...
    df["graph"] = pd.Categorical(df["graph"], sorted(df["graph"].unique(), key=lambda graph: min_numeric(df[df["graph"] == graph]["time_sec"])))

    duplicates = df[df.duplicated(subset=["graph", "algo"], keep=False)]
    if not duplicates.empty:
        warnings.warn("Duplicate entries found. Repeated entries will be ignored:\n" f"{str(duplicates)}")
    df = df.drop_duplicates(subset=["graph", "algo"])

    s_edges_df = df.pivot(index="graph", columns="algo", values="s_edges").sort_index()
    s_edges_df.columns = [f"{col} (HAS KNOWN BUGS)" if "pocr" in col.lower() else col for col in s_edges_df.columns]
    ...
    pprint_df(ram_df, title=f" RAM, GB (grammar '{grammar}') ")
    print()
    time_df = df.pivot(index="graph", columns="algo", values="time_sec").sort_index()
    pprint_df(time_df, title=f" TIME, SEC (grammar '{grammar}') ")
    print()
    print()
...
def display_results(result_files_paths: List[Path]) -> None:
    ...
    df = pd.concat([reduce_result_file_to_one_row(result_file_path) for result_file_path in result_files_paths], ignore_index=True)
...
def main(raw_args: List[str]):
    parser = argparse.ArgumentParser(description="Evaluates all vertex pairs " "Context-Free Language Reachability (CFL-R) algorithms.")

    parser.add_argument("algo_config", type=str, help="Path to the algo-config csv file.")
    parser.add_argument("data_config", type=str, help="Path to the data-config csv file.")
    parser.add_argument("result_path", type=str, help="Path to save the results.")
    parser.add_argument("--rounds", type=int, default=1, help="Number of rounds to run each configuration.")
    parser.add_argument("--timeout", type=int, default=None, help="Timeout for each run in seconds.")

    args = parser.parse_args(raw_args)
    eval_all_pairs_cflr(
        algo_config=Path(args.algo_config), data_config=Path(args.data_config), result_path=Path(args.result_path), rounds=args.rounds, timeout_sec=args.timeout
    )
