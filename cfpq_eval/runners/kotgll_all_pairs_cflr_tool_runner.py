    ...
    def base_command(self) -> Optional[str]:
        ...
        if graph.block_matrix_space.block_count > 1:
            exploded_grammar_path = (
                self.grammar_path.parent / "kotgll" / self.grammar_path.stem / self.graph_path.stem / (self.grammar_path.stem + grammar_path.suffix)
            )
            self._explode_grammar(grammar_path, exploded_grammar_path, graph.block_matrix_space.block_count)
    ...
    def parse_results(self, process: subprocess.CompletedProcess[str]) -> CflrToolRunResult:
        for line in process.stdout.split("\n"):
            if line.startswith("benchmark::"):
                parts = line.split()
                return CflrToolRunResult(s_edges=int(parts[-2]), time_sec=float(parts[-1]), ram_kb=self.parse_ram_usage_kb(process))
