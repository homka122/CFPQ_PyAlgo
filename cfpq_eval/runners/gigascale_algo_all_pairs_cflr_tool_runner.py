...
from cfpq_eval.runners.all_pairs_cflr_tool_runner import AbstractAllPairsCflrToolRunner, CflrToolRunResult, IncompatibleCflrToolError
...
class GigascaleAllPairsCflrToolRunner(AbstractAllPairsCflrToolRunner):
    ...
    def base_command(self) -> Optional[str]:
        ...
        with open(alloc_path, "w") as alloc_file, open(assign_path, "w") as assign_file, open(load_path, "w") as load_file, open(store_path, "w") as store_file:
            ...
            with open(self.graph_path, "r") as graph_file:
                for line in graph_file:
                    ...
                    if label == "alloc":
                        ...
                        assert "_r" in label, f"Label '{label} must be 'alloc', 'assign' " f"or 'load_i' or 'store_i' or contain '_r"
    ...
    def parse_results(self, process: subprocess.CompletedProcess[str]) -> CflrToolRunResult:
        ...
        return CflrToolRunResult(s_edges=int(vpt), time_sec=float(tc_time), ram_kb=self.parse_ram_usage_kb(process))
