...
from cfpq_eval.runners.kotgll_all_pairs_cflr_tool_runner import KotgllAllPairsCflrToolRunner
from cfpq_eval.runners.legacy_matrix_all_pairs_cflr_tool_runner import LegacyMatrixAllPairsCflrToolRunner
...
def run_appropriate_all_pairs_cflr_tool(algo_settings: str, graph_path: Path, grammar_path: Path, timeout_sec: Optional[int]) -> CflrToolRunResult:
    return (
        {
        ...
        .get(algo_settings, PyAlgoAllPairsCflrToolRunner)(algo_settings, graph_path, grammar_path, timeout_sec)
        .run()
    )
