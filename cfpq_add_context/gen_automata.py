...
from cfpq_add_context.labels import (
    SIGMA,
    SIGMA_WITHOUT_CONTEXTS,
    SIGMA_WITHOUT_OPEN_CONTEXTS,
    ALL_OPEN_CONTEXTS,
    mk_open_context_from_pass,
    mk_open_context_from_ret_r,
    mk_close_context_from_ret,
    mk_close_context_from_pass_r,
)
from cfpq_add_context.utils import print_matrix_to_dot


def generate(number_of_contexts: int, depth: int, is_log=False) -> Matrix:
    ...
    for level in range(0, depth):
        ...
        for _from in current_level:
            ...
            pos += 1
            for i, _to in enumerate(range(batch_start, batch_start + number_of_contexts)):
                edges.append((_from, _to, mk_open_context(i)))
                edges.append((_to, _from, mk_close_context(i)))
            # edges = edges + [
            #     edg
